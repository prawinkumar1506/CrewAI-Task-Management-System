...
def assign_task_tool(task_assignment_data: str) -> str:
    """Assign a task to a user or find a task for a user"""
    from app.models.sample_data import SampleUser, SampleUserTask
    from datetime import datetime, timedelta
    import json
    ...
    try:
        ...
        # If user_id provided without task_id - find suitable task
        if user_id and not task_id:
            ...
            if user.current_ongoing_tasks >= user.max_concurrent_tasks:
                return json.dumps({
                    "action": "assign_task",
                    "error": f"User {user_id} is at max capacity",
                    "success": False
                })

            # Find first pending task matching user skills
            pending_tasks = SampleUserTask.objects(status='pending')
            suitable_task = None
            for task in pending_tasks:
                if all(skill in user.skills for skill in task.required_skills):
                    suitable_task = task
                    break
            ...
            if not suitable_task:
                return json.dumps({
                    "action": "assign_task",
                    "error": "No suitable pending tasks found",
                    "success": False
                })

            # Update task
            suitable_task.user_id = user_id
            suitable_task.status = 'in_progress'
            suitable_task.due_date = datetime.now() + timedelta(days=7)  # Default 7-day deadline
            suitable_task.save()
            ...
            return json.dumps({
                "action": "assign_task",
                "task_id": suitable_task.task_id,
                "assigned_user": user_id,
                "due_date": suitable_task.due_date.isoformat(),
        ...
        return json.dumps({
            "action": "assign_task",
            "error": "Must provide either task_id or user_id",
            "success": False
        })
