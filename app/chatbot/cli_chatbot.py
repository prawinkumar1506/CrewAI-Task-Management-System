...
# from app.agents.supervisor_agent import supervise
# from app.models.sample_data import SampleUser, SampleUserTask
# from app.agents.crewai_integration import TaskAssignmentCrew
# import re
# from crewai import Agent, Task, Crew, Process, LLM
# from crewai.tools import tool
# def interactive_loop():
#     # Initialize the crew instance
#     crew = TaskAssignmentCrew()
#
#     print("Task Management CLI Chatbot")
#     while True:
#         print("\nOptions:")
#         print("1. Assign a task")
#         print("2. Reassign a task")
#         print("3. Supervise (auto assign/reassign)")
#         print("4. List users")
#         print("5. List tasks")
#         print("6. Exit")
...
#             result = crew.assign_task(task_id)
#             if "Error: User" in result and "is busy" in result:
#                 user_id = re.search(r"User (U\d{4})", result).group(1)
#                 result = handle_busy_user(task_id, user_id)
#             print(result)
#         elif choice == '2':
#             task_id = input("Enter task ID: ")
#             reassign_type = input("Reassignment type:\n1. Auto-reassign\n2. Manual reassign\nChoice (1-2): ")
#
#             if reassign_type == '2':
#                 new_user = input("Enter new user ID: ")
#                 due_days = input("New due date in days (optional): ")
#                 result = reassign_task(
#                     task_id,
#                     force_user_id=new_user,  # Changed parameter name
#                     new_due_days=int(due_days) if due_days else None  # Need to handle this in reassign_task
#                 )
#             else:
#                 result = reassign_task(task_id)
#                 print(result)
#         elif choice == '3':
#             supervise()
#             print("Supervisor completed checks.")
#         elif choice == '4':
#             '''users = SampleUser.objects()
#             for user in users:
#                 print(f"{user.user_id}: {user.username} ({user.availability_status}, {user.current_ongoing_tasks}/{user.max_concurrent_tasks})")
# '''
#
#
#             users = SampleUser.objects()
...
#                     status = "🚦 BUSY"
#
#                 print(f"{user.user_id}: {user.username} {status} | Workload: {workload}")
#
#         elif choice == '5':
#             tasks = SampleUserTask.objects()
#             print(tasks)
#             for task in tasks:
#                 status_indicator = "✅" if task.status == 'completed' else "⌛"
#                 print(f"{task.task_id}: {task.name} {status_indicator} ({task.status}, assigned to {task.user_id})")
#         elif choice == '6':
#             break
#         else:
#             print("Invalid choice, try again.")
#
# def handle_busy_user(task_id: str, user_id: str):
#     print(f"\n⚠️  User {user_id} is busy/overloaded. Choose:")
#     print("1. Assign anyway with deadline extension")
#     print("2. Choose next best candidate")
#     print("3. Cancel assignment")
#     choice = input("Enter choice (1-3): ")
#
#     if choice == '1':
#         days = int(input("Extend deadline by how many days? "))
#         return reassign_task(task_id, user_id, new_due_days=days)
#     elif choice == '2':
#         return crew.assign_task(task_id)  # Get next candidate
#     else:
